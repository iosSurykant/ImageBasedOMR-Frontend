...
const TabletSidebar = ({ routes = [], setIsTabCollapsed, isTabCollapsed }) => {
  ...
  const renderNavLinks = routes.filter((route) => route.showInSidebar === true);
    ...
    >
      ...
      <div
        style={{
          ...
          height: "280px",
          borderRadius: "50%",
          background: "rgba(182, 151, 255, 0.3)",
          filter: "blur(30px)",
          pointerEvents: "none",
          zIndex: 0,
        }}
        ...
        {renderNavLinks.map((item, index) => {
          ...
          return (
            <NavLink
              ...
              onMouseEnter={(e) => {
                if (!isScanning && !e.currentTarget.classList.contains("active")) {
            ...
            >
              <span
                className="drawer-icon d-flex justify-content-center text-lg"
      ...
      <div className="d-flex mt-3 px-3" style={{ gap: '8px', cursor: 'pointer' }}>
        <img
        ...
        <div className="text-left">
          <div className="font-weight-bold text-dark m-0" style={{ fontSize: '0.85rem', lineHeight: '1.2' }}>
            Admin Client
          </div>
          <small className="text-muted d-block" style={{ fontSize: '0.70rem' }}>
            Shivam
          </small>
        </div>
        <MdKeyboardArrowDown  />
