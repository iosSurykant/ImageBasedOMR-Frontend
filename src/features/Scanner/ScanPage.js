...
const ScanPage = () => {
  ...
  const extractTempByUser = (fileName = "", empid, templateId) => {
    if (fileName.includes("##")) {
      const [templateNames, extractEmpId] = fileName.split("##") || [];
      if (extractEmpId === empid) {
        return { templateNames, empid, templateId };
      }
    }
    return null;
  };
  ...
  const TemplateOption = template
    .map((item) => extractTempByUser(item.fileName, empid, item.id))
    .filter(Boolean);
  // .map((item) => item.templateNames);
